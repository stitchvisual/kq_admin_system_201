...
/**
 * Format "HH:mm" (24h) as a friendly en-AU time (e.g. "9:00 am").
 */
export function formatHHmmAU(hhmm: string): string {
  const [hs, ms] = hhmm.split(':');
  const h = Number(hs);
  const m = Number(ms);
  if (Number.isNaN(h) || Number.isNaN(m) || hs === undefined || ms === undefined) return hhmm;
